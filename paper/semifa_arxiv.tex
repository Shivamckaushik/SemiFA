...
A single FA report typically requires 2--4 hours of domain expert time~\cite{kla2023klarity,amat2023actionable}.
...
The semiconductor manufacturing process involves hundreds of sequential steps---lithography, etching, chemical-mechanical planarization (CMP), chemical vapor deposition (CVD), ion implantation, wire bonding, and dicing---each of which can introduce defects that reduce die yield.
When defects are detected by inline inspection tools such as KLA Surfscan or Applied Materials SEMVision, a failure analysis (FA) workflow is initiated.
An experienced engineer examines high-resolution inspection images under scanning electron microscopy (SEM) or optical microscopy, cross-references the defect morphology against historical cases, queries equipment alarm logs, and synthesizes a structured report documenting defect classification, root cause hypotheses, severity assessment, and corrective action recommendations.

This manual process presents three fundamental challenges.
First, a single FA report requires 2--4 hours of domain expert time~\cite{kla2023klarity,amat2023actionable}, creating a bottleneck in high-volume manufacturing (HVM) fabs that process thousands of wafers daily.
...
Commercial tools such as KLA KLARITY~\cite{kla2023klarity} and Applied Materials Actionable Insight~\cite{amat2023actionable} perform automated defect classification but do not generate natural language reports, do not fuse equipment telemetry into their reasoning, and remain proprietary and closed-source.
...
Nakazawa and Kulkarni~\cite{nakazawa2018anomaly} employed variational autoencoders for unsupervised anomaly detection on wafer maps.
...
Jeong et al.~\cite{jeong2023winclip} proposed WinCLIP for zero-/few-shot anomaly detection using CLIP~\cite{radford2021clip} embeddings, demonstrating strong transfer to manufacturing surface defect localisation.
...
Because no public labeled dataset covers these classes, we generate them synthetically, parameterizing their spatial distributions based on defect morphologies documented in published semiconductor process engineering literature and SEMI standards.
...
All FA narratives in \semidataset{} are therefore authored by the first author, drawing on nine years of semiconductor manufacturing industry experience spanning wafer inspection, process metrology, and equipment engineering domains.

The three sources are:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Procedural synthesis (3 classes, 340 images).}
    ...
    We intentionally limit the subset rather than using all available images because \semidataset{} is a \textit{VLM instruction-tuning dataset}, not a classification benchmark: each image must be paired with a manually written FA narrative, which bounds the feasible annotation volume.
    \Cref{tab:wm811k_mapping} shows the label taxonomy mapping.

    \item \textbf{MixedWM38~\cite{wang2020mixedwm38} (1 class, 140 images).}
...
\Cref{tab:source_counts} summarises the image counts per source and class.
...
Descriptions are written by the first author, who has nine years of semiconductor manufacturing industry experience spanning wire bonding, die attach, wafer inspection, and process metrology domains across multiple high-volume manufacturing environments.
...
The dedicated DINOv2 accuracy analysis (Table~\ref{tab:per_class}) reports 92.1\% as the result of the full training run in the original Colab notebook; the 2.1 point difference reflects normal variance in MLP random initialisation across runs.

\begin{table}[t]
    \centering
    \caption{Comparison of visual encoder approaches on \semidataset{} validation split (140 images). All models trained and evaluated on identical data splits; no literature-reported numbers are used.}
...
\begin{table}[t]
    \centering
    \caption{Per-node latency breakdown (median of 3 runs, NVIDIA A100-SXM4-40\,GB, PyTorch 2.10, LLaVA-1.6 in 4-bit NF4).}
    \label{tab:latency}
    \begin{tabular}{@{}lcc@{}}
        \toprule
...
The total pipeline latency of 48.4 seconds represents a reduction from the 2--4 hour manual process~\cite{kla2023klarity,amat2023actionable} to under one minute---a speedup of approximately 150--300$\times$.
...
The RootCauseAnalyzer (24.6\%, 11.9\,s) ranks second due to the multi-hypothesis generation with longer output sequences (350 tokens).
...
This suggests that future optimisation should focus on inference acceleration (e.g., speculative decoding, continuous batching, or parallelising independent agent nodes) rather than data retrieval or report assembly.
...
For 5 representative validation cases spanning five defect classes, we evaluate the RootCauseAnalyzer under four conditions:
(1)~\textbf{Full \semifa{}}: visual description + SECS/GEM telemetry + Qdrant retrieval (top-5 similar defects);
(2)~\textbf{No Retrieval}: visual description + telemetry only (Qdrant query disabled);
(3)~\textbf{No Telemetry}: visual description + retrieval only (telemetry context omitted);
(4)~\textbf{Baseline}: visual description only (image-only context, no telemetry, no retrieval).

Each generated root cause hypothesis is scored by GPT-4o~\cite{openai2023gpt4v} as an automated judge~\cite{zheng2023judging} on three criteria, each rated 1--5: \textit{specificity} (references to concrete equipment state or identifiers), \textit{actionability} (clarity and immediacy of corrective guidance), and \textit{grounding} (logical consistency between evidence and hypothesis).
...
Removing telemetry reduces the composite by 0.27 points (3.60 $\rightarrow$ 3.33), confirming that SECS/GEM alarm history and equipment parameter drift are the primary drivers of hypothesis specificity and grounding.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}p{0.8cm}p{1.8cm}p{1.8cm}p{1cm}p{1cm}p{4.2cm}p{4.2cm}@{}}
        ...
        \midrule
        1 & KLA-T800-01 & scratch & 0.91 & MAJOR & Linear track across 12 die consistent with mechanical contact; vacuum chuck pressure anomaly logged 45\,min prior to inspection & Inspect and replace vacuum chuck gasket; verify chuck flatness within $\pm$2\,$\mu$m specification \\
        \addlinespace
        2 & AMAT-CVD-03 & center\_cluster & 0.84 & MAJOR & Defect concentration within 25\% radius correlates with CVD chamber temperature gradient; similar pattern in lot LOT-2024-052 & Recalibrate showerhead spacing; increase edge zone heater setpoint by 3$^{\circ}$C \\
        \addlinespace
        3 & TEL-Etch-07 & ring\_pattern & 0.79 & MAJOR & Annular band at 60--75\% radius matches spin-coat edge-bead removal failure; RF impedance alarm logged during prior etch step & Verify edge-bead removal nozzle position; clean solvent delivery line \\
        \addlinespace
        4 & KNS-WB-12 & edge\_crack & 0.88 & MAJOR & Peripheral die damage consistent with dicing blade wear; blade lifetime at 94\% of scheduled replacement interval & Replace dicing blade; reduce dicing feed rate by 10\% for remaining lots \\
        \addlinespace
        5 & ASML-Litho-02 & no\_defect & 0.97 & NONE & No defect pattern detected; uniform yield map with $>$98\% good die; all equipment parameters within specification & No corrective action required; continue standard monitoring \\
        \bottomrule
    \end{tabular}
\end{table*}
...
This outcome establishes a clear empirical boundary: \semidataset{} with 790 training examples is sufficient to train a lightweight DINOv2 classification head (200K parameters, 92.1\% accuracy) but insufficient to fine-tune a 7B-parameter VLM via QLoRA without substantial overfitting.
Prior domain-adaptation work on comparably sized VLMs (LLaVA-Med~\cite{li2024mllm}, GeoChat) has consistently required 5{,}000--100{,}000 domain-specific instruction pairs to achieve stable fine-tuning.
...
Third, the FA narratives in \semidataset{} are authored by a single annotator; inter-annotator agreement studies would strengthen the dataset contribution.
...
Experimental evaluation on an NVIDIA A100-SXM4-40\,GB GPU demonstrated 92.1\% defect classification accuracy (macro F1 = 0.917) on 140 validation images, complete report generation in 48 seconds (a 150--300$\times$ speedup over the 2--4 hour manual process~\cite{kla2023klarity,amat2023actionable}), and a $+$0.86 composite improvement in root cause reasoning quality (GPT-4o judge, 1--5 scale) from multi-modal fusion over an image-only baseline.
...
This work is conducted as part of the M.Tech in Artificial Intelligence program at the Indian Institute of Technology Jodhpur.
The author thanks the School of Artificial Intelligence \& Data Science, IIT Jodhpur, for computational resources and academic guidance.
...
OpenAI, ``GPT-4V(ision) system card,'' OpenAI, Tech. Rep., Sep. 2023.
...
KLA Corporation, ``KLARITY: Automated defect classification and process control,'' 2023. [Online]. Available: \url{https://www.kla.com}

\bibitem{amat2023actionable}
Applied Materials, ``Actionable Insight: AI-powered process control,'' 2023. [Online]. Available: \url{https://www.appliedmaterials.com}
