...
\begin{table}[t]
    \centering
    \caption{Per-node latency breakdown (median of 3 runs, NVIDIA A100-SXM4-40\,GB, PyTorch 2.1.0, LLaVA-1.6 in 4-bit NF4).}
    \label{tab:latency}
    \begin{tabular}{@{}lcc@{}}
        \toprule
...
Equipment telemetry context consists of representative simulated SECS/GEM alarm logs --- timestamp-tagged parameter readings and alarm events formatted according to the SECS-II message structure~\cite{semi_e5} --- consistent with the system's default simulator mode (\texttt{SECSGEM\_SIM=true}).
